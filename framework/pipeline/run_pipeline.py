...
Usage:
    python -m framework.pipeline.run_pipeline --molecule gly
    python -m framework.pipeline.run_pipeline --molecule gly --basis cc-pvdz --casscf
"""
...
from .geometry import MoleculeGeometry, get_glycine_geometry, load_geometry_from_h5
...
def run_pipeline(
    ...
    Args:
        molecule: Molecule identifier ("gly" for glycine).
        basis: Basis set for calculation.
        h5_path: Path to H5 file with geometry (optional).
        run_casscf: Whether to run CASSCF in addition to CASCI.
        run_basis_comparison: Whether to run STO-3G comparison.

    Returns:
        Dict with all results: geometry, diagnostics, active_space, hamiltonian.
    """
    t0 = time.time()
    ...
    elif molecule.lower() in ("gly", "glycine"):
        geometry = get_glycine_geometry()
    else:
        raise ValueError(
            f"Unknown molecule '{molecule}'. Provide --h5-path or use 'gly'."
        )

    print(f"  Molecule: {geometry.name} ({geometry.formula})")
    print(f"  Atoms: {geometry.n_atoms}, Electrons: {geometry.n_electrons}")

    # --- Step 1: Orbital analysis ---
    print("\n[2/4] Running HF + MP2 orbital analysis...")
    t1 = time.time()
    diagnostics = run_orbital_analysis(
        geometry, basis=basis, run_basis_comparison=run_basis_comparison,
    )
    print(f"  Completed in {time.time() - t1:.1f}s")
    print(diagnostics.summary())

    # --- Step 2: Active space validation ---
    print("\n[3/4] Validating active space with CASCI...")
    t2 = time.time()
    active_space = validate_active_space(
        geometry, diagnostics, run_casscf=run_casscf,
    )
    print(f"  Completed in {time.time() - t2:.1f}s")
    print(active_space.summary())

    # --- Step 3: Qubit Hamiltonian ---
    ...
    if active_space.casscf_energy is not None:
        print(f"CASSCF energy:         {active_space.casscf_energy:.10f} Ha")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Active Space Pipeline for Qubit Hamiltonian Generation"
    )
    parser.add_argument(
        "--molecule", "-m", default="gly",
        help="Molecule identifier (default: gly)",
    )
    parser.add_argument(
        "--basis", "-b", default="cc-pvdz",
        help="Basis set (default: cc-pvdz)",
    )
    parser.add_argument(
        "--h5-path", default=None,
        help="Path to H5 file with molecular geometry",
    )
    parser.add_argument(
        "--casscf", action="store_true",
        help="Also run CASSCF orbital optimization",
    )
    parser.add_argument(
        "--basis-comparison", action="store_true",
        help="Run STO-3G comparison calculation",
    )
